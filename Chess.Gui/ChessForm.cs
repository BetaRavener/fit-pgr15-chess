...
namespace Chess.Gui
{
    public partial class ChessForm : Form, IProgress<Tuple<int, int>>
    {
        ...
        private bool _antialiasFactorChanged;
        ...
        private void RenderButton_Click(object sender, EventArgs e)
        {
            ...
            {
                RenderButton.BackColor = Color.OrangeRed;
                Render();
            }
            else
            {
                RenderButton.BackColor = Color.YellowGreen;
        ...
        private void RenderView_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            {
                _lightPos += new Vector3d(LightSensitivity*dX, LightSensitivity*dY, 0);
                _lightChanged = true;
            }
            else if (e.Button == MouseButtons.Left)
            {
                // Invert Y axis
                _raytracer.Eye.RotateRelative(new Vector2d(RotationSensitivity*dY, -RotationSensitivity*dX));
                _viewChanged = true;
                _rotating = true;
            }
            else if (e.Button == MouseButtons.Middle)
            {
                _raytracer.Eye.MoveRelative(new Vector3d(MoveSensitivity*dX, -MoveSensitivity*dY, 0));
                _viewChanged = true;
                _rotating = true;
            }
