...
namespace CSG.Noise
{
    public class ValueNoise : Noise
    {
        ...
        private static int[] values = Enumerable.Repeat(0, Tabsize).ToArray();

        private static int[] permutation = new int[Tabsize]
        {
        ...
        private int PERM(int x) { return permutation[x & (Tabsize - 1)]; }
        private int INDEX(int x, int y, int z) { return PERM(x + PERM(y + PERM(z))); }
        ...
        {
            int X = (int)Math.Floor(x) & (Tabsize - 1);
            int Y = (int)Math.Floor(y) & (Tabsize - 1);
            int Z = (int)Math.Floor(z) & (Tabsize - 1);
            ...
            for (int k = -1; k <= 2; k++)
            {
                ...
                {
                    for (int i = -1; i <= 2; i++)
                        xknots[i + 1] = values[INDEX(X + i, Y + j, Z + k)];
                    yknots[j + 1] = NoiseUtilities.Spline(x, 4, xknots);
                }
                zknots[k + 1] = NoiseUtilities.Spline(y, 4, yknots);
            }
            return NoiseUtilities.Spline(z, 4, zknots);
        }
    }
}