...
namespace CSG
{
    ...
    public class RangeShape
    {
        ...
        public void SetLeft(double value, Shapes.Shape node)
        {
            Left = value;
            LeftNode = node;
        }
        ...
        /// <param name="node">Node that creates edge.</param>
        public void SetRight(double value, Shapes.Shape node)
        {
            Right = value;
            RightNode = node;
        }
        ...
        public static RangeShape operator *(RangeShape a, RangeShape b)
        {
            if (a.Left > b.Left)
            {
                if (a.Right <= b.Right)
                {
                    return new RangeShape(a.Left, a.Right, a.LeftNode, a.RightNode);
                }
                else
                {
                    return new RangeShape(a.Left, b.Right, a.LeftNode, b.RightNode);
                }
            }
            else
            {
                if (a.Right <= b.Right)
                {
                    return new RangeShape(b.Left, a.Right, b.LeftNode, a.RightNode);
                }
                else
                {
                    return new RangeShape(b.Left, b.Right, b.LeftNode, b.RightNode);
                }
            }
        }
    }
